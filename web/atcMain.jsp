    ...
    $(document).ready(function () {
        ...
        $("#hfc").on('keyup', function () { // everytime keyup event
            ...
            if (input.length >= 1) { // Minimum characters = 2 (you can change)
                $('#atcHFCSearch').html('<img src="bootstrap-3.3.6-dist/image/ajax-loader.gif" />'); // Loader icon apprears in the <div id="match"></div>
        ...
        $("#discipline").on('keyup', function () { // everytime keyup event
            ...
            if (hfc === "" || hfc === null) {
                ...
                if (input.length >= 1) { // Minimum characters = 2 (you can change)
                    $('#atcDisciplineSearch').html('<img src="bootstrap-3.3.6-dist/image/ajax-loader.gif" />'); // Loader icon apprears in the <div id="match"></div>
                    var dataFields = {input: input}; // We pass input argument in Ajax
                    $.ajax({
                        type: "POST",
        ...
        $("#subdiscipline").on('keyup', function () { // everytime keyup event
            ...
            if (hfc === "" || hfc === null) {
                ...
                if (input.length >= 1) { // Minimum characters = 2 (you can change)
                    $('#atcSubDisciplineSearch').html('<img src="bootstrap-3.3.6-dist/image/ajax-loader.gif" />'); // Loader icon apprears in the <div id="match"></div>
                    var dataFields = {input: input}; // We pass input argument in Ajax
                    $.ajax({
                        type: "POST",
